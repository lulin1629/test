...
uint256 hashGenesisBlock("0xc4d0328614b2f28610fa64f37e31036b307289dd4d22986ed15f1dadff4bccc0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Tuhaocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Tuhaocoin Signed Message:\n";
...
{
    // Tuhaocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    else if (nHeight < 320001)
    { nSubsidy = 100000 * COIN;}
    else if (nHeight < 640001)
    { nSubsidy = 50000 * COIN;}
    else if (nHeight < 1280001)
    {nSubsidy =25000 * COIN;}
    else if (nHeight < 2560001)
    {nSubsidy =10000 * COIN;}
    else
    {nSubsidy =0;}
    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
   // nSubsidy >>= (nHeight / 840000); // Tuhaocoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 20 * 60; // Tuhaocoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Tuhaocoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Tuhaocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Tuhaocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Tuhaocoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc4d0328614b2f28610fa64f37e31036b307289dd4d22986ed15f1dadff4bccc0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Today,The Tuhao coins is building to world for now...";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1388823120;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085228618;
        ...
        assert(block.hashMerkleRoot == uint256("0xd0f8e03d0dab29c9fc1d72d7edb57843127602ca453003cf217dbbbb359c284d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Tuhaocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TuhaocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TuhaocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TuhaocoinMiner(CWallet *pwallet)
{
    printf("TuhaocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Tuhaocoin-miner");
    ...
    try { loop {
        ...
        printf("Running TuhaocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TuhaocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TuhaocoinMiner, pwallet));
}
